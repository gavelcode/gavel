...
func NewHandler(lister Finder) *Handler {
	if lister == nil {
		panic("query/pleading/list: lister must not be nil")
	}
	return &Handler{lister: lister}
}

func (h *Handler) Execute(ctx context.Context, q Query) (Result, error) {
	items, total, err := h.lister.ListByProject(ctx, q.TenantID(), q.ProjectID(), q.Status(), q.Gavelspace(), q.Limit(), q.Offset())
	if err != nil {
		return Result{}, err
	}
